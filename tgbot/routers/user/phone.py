...
from tgbot.keyboards.z_all_inline import back_search, choose_phone, not_money
...
from tgbot.utils.states import SearchPhone

router_phone = Router()


@router_phone.callback_query(Text(text='choose_phone'))
async def choose_search_phone(call: CallbackQuery, state: FSM, user: UserDB):
    await state.clear()
    price = float(get_prices()['phone'])
...
@router_phone.callback_query(Text(text='start_search_phone'))
async def search_phone(call: CallbackQuery, state: FSM, user: UserDB):
    ...
    if price > user.ballance:
        ...
        return
    await state.set_state(SearchPhone.number)
    await state.update_data(type_request="phone")
    text = "Отправьте номер телефона (в формате: 79123456789)"
    await call.message.answer(text=text, reply_markup=close_this)


@router_phone.message(SearchPhone.number, F.text)
...
@router_phone.message(SearchPhone.number)
