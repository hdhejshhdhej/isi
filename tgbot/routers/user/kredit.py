...
from tgbot.keyboards.z_all_inline import back_search, choose_search_credit, not_money
...
from tgbot.utils.states import SearchCredit
...
async def search_people_credit(call: CallbackQuery, state: FSM, user: UserDB):
    ...
    if price > user.ballance:
        ...
        return
    await state.set_state(SearchCredit.lastname)
    await state.update_data(type_request="credit")
    text = "Отправьте фамилию"
    await call.message.answer(text=text, reply_markup=close_this)

@router_credit.message(SearchCredit.lastname, F.text)
async def lastname_credit(message: Message, state: FSM, user: UserDB):
    await state.update_data(lastname=message.text)
    text = "Отправьте имя"
    await message.answer(text=text, reply_markup=close_this)
    await state.set_state(SearchCredit.firstname)


@router_credit.message(SearchCredit.firstname, F.text)
async def firstname_credit(message: Message, state: FSM, user: UserDB):
    await state.update_data(firstname=message.text)
    text = "Отправьте отчество"
    await message.answer(text=text, reply_markup=close_this)
    await state.set_state(SearchCredit.middlename)


@router_credit.message(SearchCredit.middlename, F.text)
async def middlename_credit(message: Message, state: FSM, user: UserDB):
    await state.update_data(middlename=message.text)
    text = "Отправьте день рождения <i>(в формате: 01.01.1900 )</i>"
    await message.answer(text=text, reply_markup=close_this)
    await state.set_state(SearchCredit.birthday)


@router_credit.message(SearchCredit.birthday, F.text.regexp('^[0-9]{1,2}\\.[0-9]{1,2}\\.[0-9]{4}$'))
async def day_credit(message: Message, state: FSM, bot: Bot, user: UserDB):
...
@router_credit.message(SearchCredit.lastname)
@router_credit.message(SearchCredit.firstname)
@router_credit.message(SearchCredit.middlename)
@router_credit.message(SearchCredit.birthday)
async def err_credit(message: Message):
